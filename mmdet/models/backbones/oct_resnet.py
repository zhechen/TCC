...
class OctResNet(nn.Module):
    ...
    def __init__(self, depth, layers, num_classes=1000, zero_init_residual=False,
        ...
        if init_cfg is not None:
            load_checkpoint(self, init_cfg['checkpoint'])
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        outs = []
        for i in range(4):
            res_layer = getattr(self, 'layer%d'%(i+1))
            if i == 0:
                x_h, x_l = res_layer(x)
            else:
                x_h, x_l = res_layer((x_h, x_l))
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(OctResNet, self).train(mode)
        self._freeze_stages()
